...
# get rest, add an empty row at top and bottom and an empty column to left, and convert to string of 0 and 1
...
my $accessible_rolls = 0;

my $bit = $surrounding_to_one_side - 1;
while (($bit = $grid->next_set_bit(++$bit)) >= 0) {
    ++$accessible_rolls if ($grid->get_slice([($bit - $surrounding_to_one_side)..($bit+$surrounding_to_one_side+1)]) & $surrounding)->count_bits() < 4
}
say $accessible_rolls;
